...
async function geocodeVenue(key:string, venue:VenueReq):Promise<Point|null>{
  ...
  for(const q of candidates){
    ...
    const r=await fetch(u,{headers:headers(key),next:{revalidate:86400}});
    if(!r.ok) continue;
...
async function drive(key:string,a:Point,b:Point){
  try{
    ...
    const r=await fetch(u,{headers:{...headers(key),"Content-Type":"application/json"},cache:"no-store"});
...
function haversine(a:Point,b:Point){
  const R=6371,rad=(x:number)=>x*Math.PI/180,dLat=rad(b.lat-a.lat),dLng=rad(b.lng-a.lng);
