...
export default function RegisterShowPage() {
  ...
  async function uploadPoster(file: File) {
    ...
    setUploading(false);
    if (!res.ok) { setError(data.error || "포스터 업로드에 실패했습니다."); return; }
  ...
  async function submit(e: FormEvent) {
    ...
    if (!form.poster_rights_confirmed) { setError("포스터 이미지 사용 권한 확인에 동의해주세요."); return; }
    if (!form.privacy_consent) { setError("개인정보 수집·이용에 동의해주세요."); return; }
    ...
    const data = await res.json().catch(() => ({}));
    setSubmitting(false);
    if (!res.ok) { setError(data.error || "등록 신청에 실패했습니다."); return; }
    setDone(true);
    ...
    return <>
      ...
      <main className="mx-auto min-h-[70vh] max-w-3xl px-5 py-16 sm:py-24">
        <div className="rounded-3xl border border-[#eadfce] bg-white p-8 text-center shadow-sm sm:p-12">
          ...
          <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-[#725f4d]">SHOWDAY 운영팀이 내용을 확인한 후 승인된 공연만 공개합니다. 정보 보완이 필요한 경우 입력하신 연락처 또는 이메일로 연락드릴 수 있습니다.</p>
          <div className="mt-7 flex flex-wrap justify-center gap-2">
            <button onClick={() => { setForm(initialForm); setDone(false); }} className="rounded-xl border border-[#dfd2bf] px-5 py-3 text-sm font-bold text-[#5c4a38]">다른 공연 등록</button>
            <Link href="/" className="rounded-xl bg-[#241a10] px-5 py-3 text-sm font-black text-white">SHOWDAY 홈</Link>
          </div>
        </div>
      </main>
      <Footer />
    ...
    <main className="bg-[#fffaf3]">
      <section className="mx-auto max-w-5xl px-5 py-12 sm:py-16">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight text-[#241a10] sm:text-4xl">기획사·주최사 공연 직접 등록</h1>
          <p className="mt-4 text-sm leading-7 text-[#725f4d]">회원가입 없이 공연 소식을 등록할 수 있습니다. 제출 즉시 공개되지 않으며, SHOWDAY 검수 후 승인된 공연만 서비스에 노출됩니다.</p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
          <form onSubmit={submit} className="rounded-3xl border border-[#eadfce] bg-white p-5 shadow-sm sm:p-8">
            <Section title="공연 기본정보">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="공연명 *"><input className="input" value={form.title} onChange={(e)=>set("title",e.target.value)} /></Field>
                <Field label="장르"><select className="input" value={form.genre} onChange={(e)=>set("genre",e.target.value)}>{genres.map(g=><option key={g}>{g}</option>)}</select></Field>
                <Field label="공연장 *"><input className="input" value={form.venue} onChange={(e)=>set("venue",e.target.value)} /></Field>
                ...
                <div className="sm:col-span-2"><Field label="예매 링크"><input type="url" className="input" placeholder="https://..." value={form.booking_url} onChange={(e)=>set("booking_url",e.target.value)} /></Field></div>
              </div>
            </Section>
              ...
              <div className="flex flex-col gap-4 sm:flex-row">
                ...
                <div className="flex-1">
                  <label className="inline-flex cursor-pointer rounded-xl border border-[#dfd2bf] px-4 py-2.5 text-sm font-bold text-[#5c4a38] hover:border-[#b3742f]">
                    ...
                    <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" disabled={uploading} onChange={(e)=>{const f=e.target.files?.[0]; if(f) uploadPoster(f); e.target.value="";}} />
                  </label>
                  <p className="mt-2 text-xs leading-5 text-[#8a7360]">JPG · PNG · WEBP / 최대 5MB. 세로형 공식 포스터 사용을 권장합니다.</p>
                  <label className="mt-4 flex items-start gap-2 text-xs font-semibold leading-5 text-[#5c4a38]"><input type="checkbox" className="mt-1" checked={form.poster_rights_confirmed} onChange={(e)=>set("poster_rights_confirmed",e.target.checked)} />해당 포스터를 SHOWDAY에 등록·노출할 권한이 있음을 확인합니다. *</label>
                </div>
              </div>
            </Section>
            ...
            <Section title="공연 상세정보">
              <div className="space-y-4">
                <Field label="공연 소개"><textarea className="input min-h-28" value={form.synopsis} onChange={(e)=>set("synopsis",e.target.value)} /></Field>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="출연 아티스트"><input className="input" value={form.cast_info} onChange={(e)=>set("cast_info",e.target.value)} /></Field>
                  <Field label="기획·제작"><input className="input" value={form.producer} onChange={(e)=>set("producer",e.target.value)} /></Field>
                </div>
              </div>
            </Section>
            ...
            <Section title="업체 담당자 정보">
              <div className="grid gap-4 sm:grid-cols-2">
                <Field label="기획사·주최사명 *"><input className="input" value={form.agency_name} onChange={(e)=>set("agency_name",e.target.value)} /></Field>
                <Field label="담당자 연락처 *"><input className="input" placeholder="010-0000-0000" value={form.agency_contact} onChange={(e)=>set("agency_contact",e.target.value)} /></Field>
                <div className="sm:col-span-2"><Field label="담당자 이메일 *"><input type="email" className="input" placeholder="name@company.com" value={form.agency_email} onChange={(e)=>set("agency_email",e.target.value)} /></Field></div>
              </div>
              <label className="mt-4 flex items-start gap-2 text-xs font-semibold leading-5 text-[#5c4a38]"><input type="checkbox" className="mt-1" checked={form.privacy_consent} onChange={(e)=>set("privacy_consent",e.target.checked)} />등록 검수 및 연락을 위해 담당자 연락처·이메일을 수집·이용하는 데 동의합니다. *</label>
            ...
            <button type="submit" disabled={submitting || uploading} className="mt-6 w-full rounded-2xl bg-gradient-to-r from-[#f0a850] to-[#c1762d] py-4 text-base font-black text-[#241a10] shadow-sm disabled:opacity-50">{submitting ? "등록 신청 중..." : "공연 등록 신청"}</button>
          </form>
          ...
          <aside className="h-fit rounded-3xl border border-[#eadfce] bg-white p-5 shadow-sm lg:sticky lg:top-24">
            ...
            <ol className="mt-4 space-y-4 text-xs leading-6 text-[#725f4d]">
              <li><b className="text-[#241a10]">1. 직접 입력</b><br/>공식 공연정보와 포스터를 등록합니다.</li>
              <li><b className="text-[#241a10]">2. SHOWDAY 검수</b><br/>등록 내용은 우선 ‘검토중’으로 저장됩니다.</li>
              <li><b className="text-[#241a10]">3. 승인 후 공개</b><br/>관리자가 확인한 공연만 SHOWDAY에 노출됩니다.</li>
            </ol>
            <div className="mt-5 rounded-2xl bg-[#fff6e9] p-4 text-xs leading-6 text-[#725f4d]">담당자 연락처와 이메일은 검수 용도로만 사용하며 일반 사용자 화면에는 노출하지 않습니다.</div>
          </aside>
        </div>
      </section>
