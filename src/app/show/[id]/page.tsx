...
export default function ShowDetail(){
  const {id}=useParams<{id:string}>();const [d,setD]=useState<Detail|null|undefined>(undefined);const [similar,setSimilar]=useState<Show[]>([]);const [saved,setSaved]=useState(false);const [alerted,setAlerted]=useState(false);
  useEffect(()=>{setSaved(loadSet(favKey).has(id));setAlerted(loadSet(alertKey).has(id));fetch(`/api/kopis?type=detail&id=${encodeURIComponent(id)}`,{cache:"no-store"}).then(r=>r.json()).then(x=>setD(x.detail||null)).catch(()=>setD(null))},[id]);
  ...
  return <main className="min-h-screen bg-ink text-paper"><div className="mx-auto max-w-[1180px] px-6 py-8"><a href="/" className="inline-flex items-center gap-2 text-sm font-bold text-paper hover:text-gold"><ArrowIcon className="h-4 w-4 rotate-180"/>SHOWDAY</a>
    {d===undefined?<div className="mt-10 border-y border-line py-10 text-sm text-muted">공연 상세정보를 불러오는 중입니다.</div>:!d?<div className="mt-10 border-y border-line py-10"><h1 className="text-2xl font-black">공연 상세정보를 찾지 못했습니다.</h1><p className="mt-3 text-sm text-muted">공연정보가 갱신 중이거나 상세정보가 아직 등록되지 않았을 수 있습니다.</p></div>:<>
