...
export default function ShowDetail(){
  const {id}=useParams<{id:string}>();const [isMember,setIsMember]=useState(false);const [d,setD]=useState<Detail|null|undefined>(undefined);const [ended,setEnded]=useState(false);const [similar,setSimilar]=useState<Show[]>([]);const [saved,setSaved]=useState(false);const [alerted,setAlerted]=useState(false);
  ...
  const shareShow=async()=>{const shareData={title:d?.title||"SHOWDAY 공연정보",text:d?`${d.title} · ${d.period} · ${d.venue}`:"SHOWDAY 공연정보",url:window.location.href};try{if(navigator.share)await navigator.share(shareData);else{await navigator.clipboard.writeText(window.location.href);alert("공연 링크를 복사했습니다.")}}catch{}};
