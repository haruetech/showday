...
export default function ShowDetail(){
  ...
  const copyShareLink=async()=>{try{await navigator.clipboard.writeText(window.location.href)}catch{const el=document.createElement("textarea");el.value=window.location.href;el.style.position="fixed";el.style.opacity="0";document.body.appendChild(el);el.focus();el.select();document.execCommand("copy");document.body.removeChild(el)}setShareMsg("공연 링크를 복사했습니다.");setTimeout(()=>setShareMsg(""),2500)};
  const shareShow=async()=>{const shareData={title:d?.title||"SHOWDAY 공연정보",text:d?`${d.title} · ${d.period} · ${d.venue}`:"SHOWDAY 공연정보",url:window.location.href};if(navigator.share){try{await navigator.share(shareData);return}catch(err){if(err instanceof DOMException&&err.name==="AbortError")return}}await copyShareLink()};
