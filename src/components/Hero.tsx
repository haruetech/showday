...
export default function Hero() {
  ...
  return (
    <section id="show-search" className="relative scroll-mt-24 overflow-hidden border-b border-line">
      <div className="relative h-[46vh] min-h-[280px] w-full sm:h-[52vh] lg:h-[60vh]">
        <img
          src="/hero/crowd-hero.jpg"
          alt="공연장에서 두 손을 들고 환호하는 관객들"
          className="absolute inset-0 h-full w-full object-cover"
        />
        {/* 사진 아래쪽을 화이트 테마 배경(--ink)으로 서서히 녹여서, 어두운 현장 사진과 밝은 본문이 자연스럽게 이어지도록 함 */}
        <div
          className="absolute inset-0"
          style={{
            background:
              "linear-gradient(to bottom, rgba(20,12,6,0.15) 0%, rgba(20,12,6,0.55) 60%, var(--ink) 100%)",
          }}
        />
        <div className="absolute inset-x-0 bottom-0 px-6 pb-8 sm:px-10 sm:pb-10">
          <div className="mx-auto max-w-6xl">
            <p className="text-xs font-bold tracking-wide text-[#FBE8D3] sm:text-sm">
              오늘 밤, 저 손 중 하나는 당신입니다
            </p>
            <h2 className="mt-2 font-display font-black text-2xl leading-tight text-white sm:text-3xl lg:text-4xl">
              다음 무대의 주인공은
              <br />
              바로 당신일 수 있습니다
            </h2>
      ...
      <div className="relative">
        <div className="pointer-events-none absolute inset-0" aria-hidden>
          <div className="absolute -right-24 -top-8 h-96 w-96 rounded-full bg-gold/15 blur-3xl" />
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  {shown.map((show) => {
                    ...
                    const destinationUrl =
                      show.bookingUrl ??
                      (isRealKopisId ? `https://www.kopis.or.kr/mob/db/pblprfrView.do?mt20Id=${show.id}` : null);
                    const cardInner = (
                      <>
                        <div
                    ...
                    if (!destinationUrl) {
                      // 데모용 예시 데이터 — 실제로 이동할 곳이 없어 링크로 만들지 않음(클릭해도 반응 없는 죽은 링크 방지)
                      return (
                        <div
                          key={show.id}
                          className="flex flex-col overflow-hidden rounded-sm border border-line bg-ink/35 opacity-90"
                        >
                          {cardInner}
                        </div>
                      );
