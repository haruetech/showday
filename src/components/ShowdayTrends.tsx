...
export default function ShowdayTrends({ shows }: { shows: Show[] }){
  ...
  return <section id="discover" className="mx-auto max-w-[1280px] px-6 py-12">
    ...
    <div className="mt-9 space-y-11">{groups.map(g=><ConceptRow key={g.key} id={g.key==="WEEKEND"?"weekend-shows":"genre-discovery"} icon={g.icon} title={g.title} desc={g.desc} shows={g.shows}/>)}</div>
  </section>
}

function ConceptRow({id,icon:Icon,title,desc,shows}:{id:string;icon:ComponentType<{className?:string}>;title:string;desc:string;shows:Show[]}){
