  ...
  for (const key of ["perforList", "item", "items", "row", "result"]) {
    ...
    if (candidate && typeof candidate === "object") {
      ...
      const nested = asArray(
        (candidate as Record<string, unknown>).item as
...
export async function fetchCulturePortalEvents(
  options: { startDate?: string; endDate?: string; rows?: number } = {}
  ...
  try {
    ...
    const events: ShowdayEvent[] = rows.map((row, index) => {
      const sourceId =
        firstText(row, ["seq", "id", "perforInfoId", "performanceId"]) ||
        String(index);

      const title =
        firstText(row, ["title", "name", "perforName", "prfnm", "subject"]) ||
        "제목 미정";

      const start = normalizeDateText(
        firstText(row, ["startDate", "start_date", "periodStart", "from", "startDt"])
      );

      const end = normalizeDateText(
        firstText(row, ["endDate", "end_date", "periodEnd", "to", "endDt"])
      );
      ...
      const regionText = firstText(row, [
        "area",
        "areaName",
        "sido",
        "region",
      ...
      const lat = firstNumber(row, ["gpsY", "latitude", "lat"]);
      const lng = firstNumber(row, ["gpsX", "longitude", "lng", "lot"]);

      return {
        id: makeEventId("CULTURE_PORTAL", sourceId, title, start),
        source: "CULTURE_PORTAL",
        sourceId,
        title,
        category: inferCategory(`${realm} ${title}`),
        subcategory: realm,
        region: regionText.split(" ")[0],
        district: regionText.split(" ")[1],
        venue,
        startDate: start,
        endDate: end,
        ...
        target: firstText(row, ["target", "useTarget", "age"]),
        officialUrl: firstText(row, ["url", "link", "homepage"]),
        imageUrl: firstText(row, ["thumbnail", "image", "imgUrl", "mainImg"]),
        organizer: firstText(row, ["host", "organizer", "agency"]),
        lat,
        lng,
        raw: row,
      };
